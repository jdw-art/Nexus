...
class MemoryTool(Tool):
    ...
    def _add_memory(
        ...
        添加记忆
        :param content: 记忆内容
        :param memory_type: 记忆类型：working(工作记忆), episodic(情景记忆), semantic(语义记忆), perceptual(感知记忆)
        :param importance: 重要性分数，0.0-1.0
        :param file_path: 感知记忆：本地文件路径（image/audio）
        :param modality: 感知记忆模态：text/image/audio（不传则按扩展名推断）
        :return: 执行结果
        """
        metadata = {}
        try:
            ...
            memory_id = self.memory_manager.add_memory(
                ...
                metadata=metadata,
                auto_classify=False     # 禁止自动分类，使用明确指定的类型
            )
