...
from nexus.core import NexusLLM
from nexus.memory.rag.pipeline import create_rag_pipeline
from nexus.tools import Tool, ToolParameter
from nexus.tools.base import tool_action
...
class RAGTool(Tool):
    ...
    def run(self, parameters: Dict[str, Any]) -> str:
        """
        执行工具（非展开方式）
        :param parameters: 工具参数字典，必须包含action参数
        :return: 执行结果字符串
        """
    ...
    def _add_document(
        ...
        :param namespace: 知识库命名空间（用于隔离不同项目）
    ...
    def _search(
        ...
        :param query: 搜索查询词
        :param limit: 返回结果数量
        :param min_score: 最低相关度分数
        :param enable_advanced_search: 是否启用高级搜索（MQE、HyDE）
        :param max_chars: 每个结果最大字符数
        :param include_citations: 是否包含引用来源
        :param namespace: 知识库命名空间
        :return: 搜索结果
        """
        try:
    ...
    def _ask(
        ...
        :param question: 用户问题
        :param limit: 检索结果数量
        :param enable_advanced_search: 是否启动高级搜索
        :param include_citations: 是否包含引用来源
        :param max_chars: 每个结果最大字符数
        :param namespace: 知识库命名空间
        :return: 智能回答结果
