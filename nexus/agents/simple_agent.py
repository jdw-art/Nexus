"""简单Agent，基于OpenAI原声API实现"""
import json
import re
from typing import TYPE_CHECKING, Optional, Iterator
from nexus.core import Agent, NexusLLM, Config, Message

if TYPE_CHECKING:
    from ..tools.registry import ToolRegistry

class SimpleAgent(Agent):
    """简单的Agent实现，支持可选的工具调用"""
    ...
    def _get_enhanced_system_prompt(self) -> str:
        ...
        tools_section += "1. **多个参数**：使用 `key=value` 格式，逗号分隔\n"
    ...
    def run(self, input_text: str, max_tool_iterations: int = 3, **kwargs) -> str:
        ...
        :param max_tool_iterations: 最大工具调用迭代次数（仅在启用工具时有效）
        :param kwargs: 其他参数
        :return: Agent响应
        """
        ...
        if not self.enable_tool_calling:
            ...
            self.add_message(Message(input_text, "assistant"))
            return response
